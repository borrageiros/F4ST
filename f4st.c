...
#define SWM_CTRLALTF4XKILL_ON     WM_APP+14
#define SWM_CTRLALTF4XKILL_OFF    WM_APP+15
...
int UnhookMouse();
int DisableMouse();
void UpdateXKillOverlay(POINT pt);
void HideXKillOverlay();
...
int vista = 0;
int elevated = 0;
int ctrlaltf4_xkill = 0; // If 1, Ctrl+Alt+F4 activates xkill instead of killing directly
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow) {
  { // Keep the stack clean after initialization
    ...
    // Create window
    WNDCLASSEX wnd = { sizeof(WNDCLASSEX), 0, WindowProc, 0, 0, hInst, NULL, LoadCursor(NULL, IDC_ARROW), (HBRUSH)(COLOR_WINDOW+1), NULL, APP_NAME, NULL };
    RegisterClassEx(&wnd);
    g_hwnd = CreateWindowEx(WS_EX_TOOLWINDOW|WS_EX_TOPMOST|WS_EX_LAYERED|WS_EX_TRANSPARENT, wnd.lpszClassName, NULL, WS_POPUP, 0, 0, 0, 0, NULL, NULL, hInst, NULL);
    SetLayeredWindowAttributes(g_hwnd, 0, 1, LWA_ALPHA);

    WNDCLASSEX overlay_wnd = { sizeof(WNDCLASSEX), 0, OverlayWindowProc, 0, 0, hInst, NULL, NULL, (HBRUSH)(COLOR_WINDOW+1), NULL, L"F4STOverlay", NULL };
    RegisterClassEx(&overlay_wnd);
    g_overlay_hwnd = CreateWindowEx(WS_EX_TOOLWINDOW|WS_EX_TOPMOST|WS_EX_LAYERED|WS_EX_NOACTIVATE|WS_EX_TRANSPARENT, overlay_wnd.lpszClassName, NULL, WS_POPUP, 0, 0, 0, 0, NULL, NULL, hInst, NULL);
    SetLayeredWindowAttributes(g_overlay_hwnd, 0, 128, LWA_ALPHA);
  ...
  if (nCode == HC_ACTION) {
    ...
    if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) {
      ...
      if (vkey == VK_LCONTROL) {
        ...
        // Check if Ctrl+Alt+F4 should activate xkill mode
        if (ctrlaltf4_xkill) {
          // Hook mouse for xkill mode
          HookMouse();
        }
        else {
...
LRESULT CALLBACK LowLevelMouseProc(int nCode, WPARAM wParam, LPARAM lParam) {
  if (nCode == HC_ACTION) {
    if (superkill && wParam == WM_MOUSEMOVE) {
      POINT pt = ((PMSLLHOOKSTRUCT)lParam)->pt;
      UpdateXKillOverlay(pt);
      return CallNextHookEx(NULL, nCode, wParam, lParam);
    }
...
void UpdateXKillOverlay(POINT pt) {
  if (!superkill || g_overlay_hwnd == NULL) {
    return;
  }

  HWND hwnd = WindowFromPoint(pt);

  if (hwnd == NULL || hwnd == g_hwnd || hwnd == g_overlay_hwnd) {
    return;
  }
  ...
  if (GetWindowRect(hwnd, &rect)) {
    ...
    SetWindowPos(g_overlay_hwnd, HWND_TOPMOST, rect.left, rect.top, width, height, SWP_NOACTIVATE | SWP_SHOWWINDOW);
  }
}

void HideXKillOverlay() {
  if (g_overlay_hwnd != NULL) {
    ShowWindow(g_overlay_hwnd, SW_HIDE);
  }
}
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  if (msg == WM_TRAY) {
    ...
    else if (wmId == SWM_CTRLALTF4XKILL_ON) {
      WritePrivateProfileString(L"General", L"CtrlAltF4XKill", L"1", inipath);
      SendMessage(g_hwnd, WM_UPDATESETTINGS, 0, 0);
    }
    else if (wmId == SWM_CTRLALTF4XKILL_OFF) {
      WritePrivateProfileString(L"General", L"CtrlAltF4XKill", L"0", inipath);
      SendMessage(g_hwnd, WM_UPDATESETTINGS, 0, 0);
    }
